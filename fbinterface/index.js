  ...
  constructor({ pageAccessToken, verifyToken, appSecret }) {
    try {
      if (pageAccessToken && verifyToken) {
        ...
        throw "Jedan ili više tokena/kredencijala nedostaju!";
      }
    } catch (e) {
      console.log(e);
    }
    ...
    const mode = params['hub.mode'],
      ...
      try {
        if ((mode && token) && (mode === 'subscribe' && token === this.verifyToken)) {
          console.log("Webhook je registrovan!");
          return res.send(challenge);
        } else {
          throw "Registracija webhook nije uspela!";
